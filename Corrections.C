...
void CorrectedAllPaper(TString particle, TString prod, TString rapidity, double lambda, int low, int high) {
...
//Create visual graph
    TCanvas *c1 = new TCanvas("c1","Corrected Data",0,0,700,700);

    //TPaveLabel *title = new TPaveLabel(0.15,0.94,0.9,0.98, prod+" "+particle+" at rapidity slice number "+ rapidity+" ("+polarisation+")");
   //title->SetFillColor(16);
   //title->SetTextFont(42);
   //title->Draw();

    TPad *pad3 = new TPad("pad3","pad3",0.05,0.02,1,0.33);
   TPad *pad2 = new TPad("pad2","pad2",0.05,0.31,1,0.62);
   TPad *pad1 = new TPad("pad1","pad1",0.05,0.59,1,0.98);
    pad1->Draw(); pad2->Draw(); pad3->Draw();
    ...
    Ratio->GetXaxis()->SetLabelSize(0.001);
    Ratio->GetYaxis()->SetTitleSize(0.06);
    Ratio->DrawCopy("E1 X");
    TF1 *constfunc1 = new TF1("constfunc1", constantratio, low, high, 0);
    constfunc1->Draw("same");
